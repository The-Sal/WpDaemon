...
[**Argus**](https://github.com/The-Sal/Argus) is a high-performance financial market data system that provides unified access to multiple exchanges through a custom binary protocol (Protocol 2). It follows a **dispatcher paradigm** where server processes connect to financial APIs and multiplex real-time market data to multiple clients and 
allows for **real-time order execution and management** with support for various exchanges such as Binance, PolyMarket, Capital.com, Interactive Brokers, and more.

**Example Argus Architecture:**
```
Client 1 (Python) ──┐
Client 2 (C++)    ──┼──> TCP 9972 ──> [Polymarket/IB/Binance/Capital.com Dispatcher] ──> CeX/DeX/Broker/Exchange
Client 3 (Rust)   ──┘                                    ↓
                                 Protocol 2 stream / Order Execution / Order Management
...
- WpDaemon supports all targets supported by Argus


## Releases

| Target Platform          | Architecture               | OS Version  | GLIBC |
|--------------------------|----------------------------|-------------|-------|
| **macOS**                | x86_64 + arm64 (Universal) | macOS 13.1+ | N/A   |
| **Debian 12 (Bookworm)** | aarch64, armv7             | Debian 12+  | 2.36+ |

Compiled Binaries are available in the [Releases](https://github.com/the-sal/WpDaemon/releases) section as `.zip` archives
which include the WpDaemon binary for all the above platforms. With the following structure:
...
**run.log** is generated by `build_system/script.sh` when building the respective binary.
